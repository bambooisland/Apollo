...
public class PlayerManager {
	...
	private static ObjectProperty<ObservableList<ApolloSong>> playList = new SimpleObjectProperty<>();
	private static ObjectProperty<ApolloSong> currentSong = new SimpleObjectProperty<>();
	...
	public static ObjectProperty<ApolloSong> currentSongProperty() {
		return currentSong;
	}
	...
	public static ObjectProperty<ObservableList<ApolloSong>> playListProperty() {
		return playList;
	}
	...
	private static void getNewPlayer(ApolloSong song, boolean playing) {
		...
		currentPlayer.currentTimeProperty().addListener( (value, oldValue, newValue) -> {
			...
			if (newValue.greaterThan(totalTime.get().multiply(stopTime.get())) && currentPlayer.getStatus().equals(MediaPlayer.Status.PLAYING)) {
	...
	public static void setPlaylist(ListView<ApolloSong> list) {
		ApolloSong song = list.getSelectionModel().getSelectedItem();
		if (song == null) {
			return;
		}
